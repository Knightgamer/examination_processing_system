...
function SpecialConsiderationStudentsReportTable() {
  ...
  useEffect(() => {
    // Filter students with specialConsideration set to true and show the reason
    const filteredStudents = data.filter((student) => {
      if (student.specialConsideration && student.specialConsideration.reason) {
        return (
          student.specialConsideration.reason
            .toLowerCase()
  ...
  const exportToCSV = () => {
    ...
    const fileName = "special_consideration_reports.csv";
    const headers = [
      ...
      {
        label: "Reason for Special Consideration",
        key: "Reason for Special Consideration",
      },
    ];
  ...
  const exportToExcel = () => {
    ...
    const fileName = "special_consideration_reports.xlsx";
    const ws = XLSX.utils.json_to_sheet(excelData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Semester Reports");
  ...
  const exportToPDF = () => {
    ...
    doc.save("special_consideration_reports.pdf");
  };

  return (
    <div>
      <h2 className="text-3xl font-semibold text-center mb-8">
        Special Consideration Reports
      </h2>
...
export default SpecialConsiderationStudentsReportTable;
