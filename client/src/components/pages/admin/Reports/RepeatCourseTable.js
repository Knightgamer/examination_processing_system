...
function RepeatCourseTable() {
  ...
  useEffect(() => {
    // Filter students attempting a course for the second time (having failed in the first attempt)
    const filteredStudents = data.filter((student) => {
      const courseEntries = data.filter((entry) => {
        return (
          entry.student._id === student.student._id &&
          entry.course.courseCode === student.course.courseCode &&
          entry.course.courseName === student.course.courseName
        );
      });

      // Check if the student has a grade of "F" and there are multiple course entries
      if (student.grade === "F" && courseEntries.length > 1) {
        return true;
      }
  ...
  const filteredDataForSearch = filteredData.filter((student) =>
  ...
  return (
    <div>
      ...
      <div id="datatable">
        {filteredDataForSearch.length === 0 ? (
...
export default RepeatCourseTable;
