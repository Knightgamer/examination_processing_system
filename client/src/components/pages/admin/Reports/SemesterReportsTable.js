...
function SemesterReportsTable() {
  ...
  const exportToCSV = () => {
    const csvData = [];
    Object.entries(organizedData).forEach(([semester, students]) => {
    ...
    const fileName = "semester_reports.csv";
    const headers = [
  ...
  const exportToExcel = () => {
    ...
    const fileName = "semester_reports.xlsx";
  ...
  const exportToPDF = () => {
    ...
    doc.autoTable({
      head: [
        ["Semester", "Student Name", "Course Code", "Course Name", "Grade"],
      ],
      body: pdfData,
    });

    doc.save("semester_reports.pdf");
  };
    ...
    <div>
      ...
      <div id="datatable">
        {Object.keys(organizedData).length === 0 ? (
          ...
          <div>
            {Object.entries(organizedData).map(([semester, students]) => (
              <div key={semester}>
                <h2 className="text-lg font-semibold mt-4">{semester}</h2>
                  ...
                  <tbody>
                    {students.map((student) => (
                      <tr key={student._id}>
                        ...
                        <td className="px-4 py-2 text-left">{student.grade}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export default SemesterReportsTable;
